...
parser.add_argument('--log_interval', type=int, default=100, help='number of iterations between each stdout logging, default=1')
...
dirCheckpoints = rootDir + '/code/STVAE/checkpoints1/originalVAE_final'
dirImageoutput = dirCheckpoints + '/train_oulu_img'
dirTestingoutput = dirCheckpoints + '/test_oulu_img'
dirModel = dirCheckpoints + '/model'
load_encoder = rootDir + '/code/STVAE/checkpoints/pretrain_final/model/encoder_epoch_4.pth'
load_decoder = rootDir + '/code/STVAE/checkpoints/pretrain_final/model/decoder_epoch_4.pth'
...
def rotate_train():
    ...
    best_accs = []
    best_triaccs = []
    accFilename = rootDir + '/code/STVAE/checkpoints/evaluate/originalVAE/classify_acc.txt'
    accfile = open(accFilename, 'a')
    triFilename = rootDir + '/code/STVAE/checkpoints/evaluate/originalVAE/triplet_acc.txt'
        ...
        for epoch in range(args.nepoch):
            ...
            for i, batch in enumerate(dataloader):
                ...
                if iteration_count % args.img_interval == 0:
                    visualizeAsImages(_anc.data.clone(),
                                              dirImageoutput,
                                              filename='iter_' + str(iteration_count) + '_anc_', n_sample=64, nrow=8, normalize=False)
                    visualizeAsImages(_recon_anc.data.clone(),
                                              dirImageoutput,
                                              filename='iter_' + str(iteration_count) + '_anc_recon_', n_sample=64, nrow=8, normalize=False)
                    visualizeAsImages(_pos.data.clone(),
                                      dirImageoutput,
                                      filename='iter_' + str(iteration_count) + '_pos_', n_sample=64, nrow=8,
                                      normalize=False)
                    visualizeAsImages(_recon_pos.data.clone(),
                                      dirImageoutput,
                                      filename='iter_' + str(iteration_count) + '_pos_recon_', n_sample=64, nrow=8,
                                      normalize=False)
                    visualizeAsImages(_neg.data.clone(),
                                      dirImageoutput,
                                      filename='iter_' + str(iteration_count) + '_neg_', n_sample=64, nrow=8,
                                      normalize=False)
                    visualizeAsImages(_recon_neg.data.clone(),
                                      dirImageoutput,
                                      filename='iter_' + str(iteration_count) + '_neg_recon_', n_sample=64, nrow=8,
                                      normalize=False)
                    test(k)
            ...
            for i, batch in enumerate(dataloader):
                ...
                if True:
                    visualizeAsImages(_anc.data.clone(),
                                      dirTestingoutput,
                                      filename='epoch_' + str(epoch) + '_' + str(i) + '_anc_', n_sample=64, nrow=8,
                                      normalize=False)
                    visualizeAsImages(_recon_anc.data.clone(),
                                      dirTestingoutput,
                                      filename='epoch_' + str(epoch) + '_' + str(i) + '_anc_recon_', n_sample=64, nrow=8,
                                      normalize=False)
                    visualizeAsImages(_pos.data.clone(),
                                      dirTestingoutput,
                                      filename='epoch_' + str(epoch) + '_' + str(i) + '_pos_', n_sample=64, nrow=8,
                                      normalize=False)
                    visualizeAsImages(_recon_pos.data.clone(),
                                      dirTestingoutput,
                                      filename='epoch_' + str(epoch) + '_' + str(i) + '_pos_recon_', n_sample=64, nrow=8,
                                      normalize=False)
                    visualizeAsImages(_neg.data.clone(),
                                      dirTestingoutput,
                                      filename='epoch_' + str(epoch) + '_' + str(i) + '_neg_', n_sample=64, nrow=8,
                                      normalize=False)
                    visualizeAsImages(_recon_neg.data.clone(),
                                      dirTestingoutput,
                                      filename='epoch_' + str(epoch) + '_' + str(i) + '_neg_recon_', n_sample=64, nrow=8,
                                      normalize=False)
