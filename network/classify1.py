...
dirCheckpoints = dirRoot + '/code/STVAE/checkpoints1/tevae_final_classify'
...
def predict(train_subject, test_subject):
    ...
    for i, batch in enumerate(dataloader):
        ...
        mu = z.cpu().detach().numpy().tolist()
        mu1 = z1.cpu().detach().numpy().tolist()
    ...
    clf = svm.SVC(C=grid.best_params_['C'], gamma=grid.best_params_['gamma'], decision_function_shape='ovo')
...
def rotate_train():
    ...
    for k in range(10):
        ...
        for epoch in range(5):
            ...
            select = 20000
            total_valid = 0
            dataset = Oulu_triplet.SelectTripletOuluLoader(train_split[k], select, encoder, mytransform)
            # print('# size of the current (sub)dataset is %d' % len(dataset))
            dataloader = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=True,
                                                     num_workers=4)
            encoder.train()
            ...
            for i, batch in enumerate(dataloader):
                ...
                if choice == 1:
                    ...
                    fpl_loss = fpl_loss3
                # fpl_loss = (fpl_loss1 + fpl_loss2 + fpl_loss3) / 3
                rank_loss, valid = tuplet_loss(z_anc, z_pos, z_neg)
                total_valid += valid
                avg_rank += rank_loss
                # avg_kld += kld_loss
                # avg_fpl += fpl_loss
                ...
                #     print('RECON[{}/{}][{}/{}] KLD: {:.4f} FPL: {:.4f} RANK: {:.4f} TOTAL: {:.4f} VALID: {:.1f}'.format(
                #     epoch, args.nepoch, i, len(dataloader),
                #           kld_loss, fpl_loss, rank_loss, loss, valid))
