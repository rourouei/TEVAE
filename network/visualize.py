...
dirCheckpoints = dirRoot + '/code/STVAE/checkpoints1/tevae_final11'
...
plain = False
if plain == False:
    load_encoder = dirRoot + '/code/STVAE/checkpoints1/tevae_final' +str(conf) + '/model/encoder_epoch_' + str(epo) +'.pth'
    load_decoder = dirRoot + '/code/STVAE/checkpoints1/tevae_final' +str(conf) + '/model/decoder_epoch_' + str(epo) +'.pth'
else:
    load_encoder = dirRoot + '/code/STVAE/checkpoints1/originalVAE_final/model/encoder_epoch_' + str(epo) + '.pth'
    load_decoder = dirRoot + '/code/STVAE/checkpoints1/originalVAE_final/model/decoder_epoch_' + str(epo) + '.pth'
...
select = 20000
train_subjects = []
test_subjects = [20, 35, 38, 75, 45, 7, 3, 44, 2, 8, 9, 10, 11, 23, 24, 29]
for i in range(1, 82):
    if i not in test_subjects:
        train_subjects.append(i)

test_dir = '/home/njuciairs/zmy/code/STVAE/visualize/test_subject'

def _reconstruct(dir, filenames):
    pathname = os.path.join(dir, 'recon' + str(conf) + '_' + str(epo))
    zs = []
    ...
    z = torch.stack([zi for zi in zs])
    recons = decoder(z)
    recons = transform_convert(recons)
    visualizeAsImages(recons.data.clone(),
                      '',
                      filename=pathname, n_sample=len(zs), nrow=len(zs), normalize=False)


def _reconstruct1(dir, filenames):
    for filename in filenames:
        filename = os.path.join(dir, filename)
        print(filename)
        pathname = filename[0:-5] + '_' + str(plain)
        img = Image.open(filename).convert('RGB')
        img = mytransform(img).cuda()
        img = torch.unsqueeze(img, dim=0)
        z, _, _ = encoder(img)
        recon = decoder(z)
        recon = transform_convert(recon)
        visualizeAsImages(recon.data.clone(),
                          '',
...
def reconsruction():
    for sub in test_subjects:
        if sub < 10:
            dirpath = test_dir + '/P00' + str(sub)
        else:
            dirpath = test_dir + '/P0' + str(sub)
        ...
        for root, _, fnames in os.walk(dirpath):
            ...
            if len(fnames) == 0:
                continue
            for fname in fnames:
                if fname.find('.jpeg') != -1:
                    originalFname.append(fname)
            originalFname = sorted(originalFname)
            _reconstruct(root, originalFname)
            # _reconstruct1(root, originalFname)
    ...
    for sub in test_subjects:
        ...
        for root, _, fnames in os.walk(dirpath):
            ...
            for fname in sorted(fnames):
                if fname.find('.jpeg') != -1:
                    originalFname.append(fname)
            for fname in sorted(originalFname):
                ...
                i += 1
                txt_path = root + '/tevae' + str(conf) + '_' + str(epo) + '_' + str(i) + '.txt'
                    # print(txt_path)
...
def subtraction():
    path1 = '/home/njuciairs/zmy/code/STVAE/visualize/test_subject/P002/Surprise/000.jpeg'
    path2 = path1 = '/home/njuciairs/zmy/code/STVAE/visualize/test_subject/P002/Surprise/017.jpeg'
    ...
    z2, _, _ = encoder(img2)
    sub = decoder(z1 - z2)
    sub = transform_convert(sub)
    visualizeAsImages(sub.data.clone(),
                      '',
                      filename='/home/njuciairs/zmy/code/STVAE/visualize/test_subject/P002/Surprise/sub.png', n_sample=1, nrow=1, normalize=False)


# subtraction()

path1 = '/home/njuciairs/zmy/code/STVAE/visualize/0.jpeg'
path2 = '/home/njuciairs/zmy/code/STVAE/visualize/1.jpeg'
pathname = '/home/njuciairs/zmy/code/STVAE/visualize/inter'
def _interpolate(path1, path2, n, pathname):
    img1 = Image.open(path1).convert('RGB')
    img2 = Image.open(path2).convert('RGB')
    img1 = mytransform(img1).cuda()
    img2 = mytransform(img2).cuda()
...
# _interpolate(path1, path2, 9, pathname)

def interpolate(root, filenames, n=9):
    path1 = os.path.join(root, filenames[0])
    path2 = os.path.join(root, filenames[-1])
    pathname = os.path.join(root, 'inter' + str(conf) + '_' + str(epo))
    img1 = Image.open(path1).convert('RGB')
    img2 = Image.open(path2).convert('RGB')
    img1 = mytransform(img1).cuda()
    img2 = mytransform(img2).cuda()
        ...
        for root, _, fnames in os.walk(dirpath):
            ...
            for fname in fnames:
                if fname.find('.jpeg') != -1:
                    originalFname.append(fname)
            originalFname = sorted(originalFname)
            interpolate(root, originalFname)
            # _reconstruct1(root, originalFname)
...
id1 = '/home/njuciairs/zmy/code/STVAE/visualize/0.jpeg'
expr = '/home/njuciairs/zmy/code/STVAE/visualize/1.jpeg'
id2 = '/home/njuciairs/zmy/code/STVAE/visualize/2.jpeg'
recon_path = '/home/njuciairs/zmy/code/STVAE/visualize/trans'
