...
parser.add_argument('--img_interval', type=int, default=10, help='number of iterations between each image saving, default=100')
...
dirCheckpoints = '/home/njuciairs/zmy/code/STVAE/checkpoints/stvae_rank11'
dirImageoutput = dirCheckpoints + '/train_img'
dirTestingoutput = dirCheckpoints + '/test_img'
dirModel = dirCheckpoints + '/model'
dirLoad = '/home/njuciairs/zmy/code/STVAE/checkpoints/stvae_pretrain1'
load_encoder = dirLoad + '/model/encoder_epoch_4.pth'
load_decoder = dirLoad + '/model/decoder_epoch_4.pth'
...
def tuplet_loss(anchor, close, sequence):
    delta = 3e-1 * torch.ones(anchor.size(0), device='cuda')
    # N x 5 x 100
    #print('!!!!', anchor.shape)
    ...
    far_distance = criterion_l2(anchors.expand(sequence.size(0), triplet_num, 100), sequence)
    loss = torch.max(torch.zeros(anchor.size(0), device='cuda'), (close_distance - far_distance[:, 0] + delta))

    for i in range(1, triplet_num):
        loss += torch.max(torch.zeros(anchor.size(0), device='cuda'),
                          (far_distance[:, i - 1] - far_distance[:, i] + delta))
...
def train():
    ...
    for epoch in range(args.nepoch):
        dataset = Oulu.OuluLoader(subject=train_split[0], spacing=3,transform=mytransform)
        ...
        for i, (batch, label) in enumerate(dataloader):
            ...
            _, input_emb, _ = encoder(batch[0].cuda())
            _, close_emb, _ = encoder(batch[1].cuda())

            _, far_emb, _ = encoder(torch.stack(batch[2:],dim=1).cuda().view(-1,3,64,64))
            far_emb = far_emb.view(batch_size, -1, 100)
            rank_loss = tuplet_loss(input_emb, close_emb, far_emb)
            ...
            if iteration_count % args.img_interval == 0:
                visualizeAsImages(_input.data.clone(),
                                          dirImageoutput,
                                          filename='iter_' + str(iteration_count) + '_input_', n_sample=49, nrow=7, normalize=False)
                visualizeAsImages(_recon.data.clone(),
                                          dirImageoutput,
                                          filename='iter_' + str(iteration_count) + '_recon_', n_sample=49, nrow=7, normalize=False)
        ...
        random_generates(5,epoch)
        #do testing
        dataset = Oulu.OuluLoader(subject=test_split[0], spacing=3,transform=mytransform)
        print('# size of the current (sub)dataset is %d' % len(dataset))
        dataloader = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=True,
                                                 num_workers=4)
        encoder.eval()
        ...
        for i, (batch,label) in enumerate(dataloader):
            ...
            with torch.no_grad():
                ...
                fpl_loss = 0
                for input in batch:
                    z, mu, logvar = encoder(input.cuda())
                    target_feature = descriptor(input.cuda())
                    kld_loss += -0.5 * torch.mean(1 + logvar - mu.pow(2) - logvar.exp())
                    recon = decoder(z)
                    recon_feature = descriptor(recon)
                    fpl_loss += fpl_criterion(recon_feature, target_feature)
